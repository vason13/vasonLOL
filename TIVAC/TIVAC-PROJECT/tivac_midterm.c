...
#include <stdarg.h> //include necessary libraries
#include <stdbool.h>
#include <stdint.h>
...
//Configures the UART to run at 15200 baud rate
{
    ...
    UARTStdioConfig(1, 15200, 16000000);    //enables UARTstdio baud rate, clock, and which UART to use
}
...
void main (void)
{
    ...
    while(1) //this while loop will continuously record the averaged lux inputs
    {
    ...
    usprintf (HTTP_POST, "GET /update?key=UPCJWFLFAG6JJNRN&field1=%d&headers=falseHTTP/1.1\nHostapi.thingspeak.com\nConnection:close\Accept*\*\r\n\r\n", luxAvg);
